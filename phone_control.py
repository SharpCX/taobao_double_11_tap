...
class PhoneControl():
    ...
    def input_text(self, text):
        ...
        :return:文本信息
        """
        text_to_pinyin = "".join(lazy_pinyin(text))
        command = r'adb -s ' + str(self.phone) + ' shell input text {}'.format(text_to_pinyin)
        system(command)
        return text
    ...
    @connect_phone
    def go_to_wx_home(self):
        # 先关闭再打开
        command_close = r'adb -s ' + self.phone + " shell am force-stop com.tencent.mm"
        system(command_close)
        time.sleep(2)
        command = r'adb -s ' + self.phone + " shell am start -W -n com.tencent.mm/.ui.LauncherUI"
        print(command)
        system(command)
...
class OperateAllPhone():
    ...
    def back_to_wx_home(self):
        return self.operate_all("go_to_wx_home", ())
